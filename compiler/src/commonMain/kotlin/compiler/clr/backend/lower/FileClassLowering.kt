...
internal class FileClassLowering(val context: ClrBackendContext) : FileLoweringPass {
	...
	private fun createFileClass(irFile: IrFile, fileClassMembers: List<IrDeclaration>): IrClass {
		...
		val onlyPrivateDeclarationsAndFeatureIsEnabled = false
		...
		val fileClassOrigin =
			if (!isMultifilePart || /*context.config.languageVersionSettings.getFlag(JvmAnalysisFlags.inheritMultifileParts)*/ false)
		...
		return context.irFactory.createClass(
			...
			/*context.state.factory.packagePartRegistry.addPart(
				irFile.packageFqName,
				partClassType.internalName,
		...
		parsedAnnotations != null -> {
			...
			when {
				parsedAnnotations.isMultifileClass -> JvmMultifileClassPartInfo(
					fileClassFqName = packageFqName.child(
						Name.identifier(
							JvmFileClassUtil.manglePartName(
								simpleName,
								fileName
							)
						)
...
internal class ParsedJvmFileClassAnnotations(
	val jvmName: String?,
	val jvmPackageName: FqName?,
